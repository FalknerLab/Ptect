...
def get_key_val_recur(key_name: str, nest_dict):
    ...
    if len(key_split) > 1:
        ...
        if key_split[0] in nest_dict.keys():
...
class BasicExp:
    ...
    def get_runs_from_key_val(self, key_name: str, key_val: str):
        """
        Gets runs that match a specific key-value pair.
        ...
        for i, r in enumerate(self.runs):
            if r.info.has_kv(key_name, key_val):
...
def has_key_value_recur(test_dict: dict, key: str, value: str):
    ...
    #     elif k == key and test_dict[k] == value:
    #         has_kv = True
    # return has_kv
    val = get_key_val_recur(key, test_dict)
    has_kv = val == value
    return has_kv
