...
def plot_mean_sem(data_mat: np.ndarray, ax=None, col='r', x=None):
    ...
    ax.plot(x, mean, c=col, linewidth=3)
    ax.plot(x, mean + sem, c=col, linewidth=1, alpha=0.5)
    ax.plot(x, mean - sem, c=col, linewidth=1, alpha=0.5)
...
def plot_linear_regression(x, y, ax=None):
    ...
    ax.scatter(x, y, c=range(len(x)), cmap='tab20')
    coef = np.polyfit(x, y, 1)
    poly1d_fn = np.poly1d(coef)
    test_x = np.arange(np.min(x), np.max(x), 0.1)
    y_pred = poly1d_fn(x)
    ax.plot(test_x[:, None], poly1d_fn(test_x), 'r--')
    r2 = r2_score(y, y_pred)
    return r2


def plot_marking(ptect_data_file, ax_list=None, count_thresh=0):
    urine_data = np.load(ptect_data_file)['urine_data']
    axs = ax_list
    if axs is None:
        f, axs = plt.subplots(1, 4)
    hot_u = urine_data[:, 3] == 0
    split_inds = (hot_u, ~hot_u)
    cols = ('b', 'r')
    for i in range(2):
        this_u = urine_data[split_inds[i]]
        _, first_inds, cnts = np.unique(this_u[:, 1:3], axis=0, return_index=True, return_counts=True)
        first_inds = first_inds[cnts > count_thresh]
        add_territory_circle(axs[i], 'block0')
        s = axs[i].scatter(this_u[first_inds, 1], this_u[first_inds, 2], c=this_u[first_inds, 0], marker='.', s=1,
                           cmap='plasma', rasterized=True)
        axs[2].scatter(this_u[first_inds, 1], this_u[first_inds, 2], c=cols[i], alpha=0.5, edgecolors='none',
                       marker='.', s=1, rasterized=True)
    plt.colorbar(mappable=s, cax=axs[3])
